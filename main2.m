...
time_best=arrayfun(@(i) calculation(cast_time(i,1),explode_time(i,1),velocity_smoke_x(i,1),velocity_smoke_y(i,1)),1:size)';
...
for echo=0:1:max_echo
    ...
    for i=1:size
         ...
         cast_time_v(i,1) = w * cast_time_v(i,1) + c1*cast_r1.* (cast_time_best(i,1) - cast_time(i,1)) + c2 * cast_r2 .* (all_cast_time_best - cast_time(i,1))+cast_r0;
         explode_time_v(i,1) = w * explode_time_v(i,1) + c1*explode_r1.* (explode_time_best(i,1) - explode_time(i,1)) + c2 * explode_r2 .* (all_explode_time_best - explode_time(i,1))+explode_r0;
         velocity_v(i,1) = w * velocity_v(i,1) + c1*v_r1.* (velocity_best(i,1) - velocity(i,1)) + c2 * v_r2 .* (all_velocity_best - velocity(i,1))+v_r0;
         angle_v(i,1) = w * angle_v(i,1) + c1*a_r1.* (angle_best(i,1) - angle(i,1)) + c2 * a_r2 .* (all_angle_best - angle(i,1))+a_r0;
         ...
         [time_tochoose,start_time,end_time]=multi_return_calculation(cast_time(i,1),explode_time(i,1),velocity_smoke_x(i,1),velocity_smoke_y(i,1));
